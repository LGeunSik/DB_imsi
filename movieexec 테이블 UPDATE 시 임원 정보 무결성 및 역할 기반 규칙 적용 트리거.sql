/*
[Trigger 제목]
movieexec 테이블 UPDATE 시 임원 정보 무결성 및 역할 기반 규칙 적용 트리거

[코드 동작 흐름 설명]
1) movieexec 테이블의 레코드가 UPDATE되기 전에 실행되는 BEFORE UPDATE 트리거이다.
2) 수정 대상 임원이 영화사 사장(president) 또는 영화 제작자(producer)인 경우 이름 변경을 허용하지 않는다.
3) networth가 NULL로 변경될 경우, 기존 임원들 중 최대 재산값으로 자동 보정한다.
4) networth가 증가했을 때 평균 재산보다 크고, 사장·제작자 역할이 없는 경우 임의의 영화사 사장으로 지정한다.
5) 해당 임원이 배우로 출연한 이력이 있으면 주소 필드에 경고 메시지를 추가한다.
