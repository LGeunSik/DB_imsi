/*
[Trigger 제목]
moviestar 테이블 INSERT 시 배우 기본 정보 자동 생성 및 성별 결정 트리거

[코드 동작 흐름 설명]
1) moviestar 테이블에 새로운 배우가 INSERT되기 전에 실행되는 BEFORE INSERT 트리거이다.
2) birthdate가 NULL이면 1980-01-01을 기준으로 최대 약 45년 범위 내의 임의 날짜를 생성해 설정한다.
3) address가 NULL이면 서울시 내 구·동·번지를 난수로 조합하여 임의의 주소를 생성한다.
4) gender가 NULL이면 해당 배우보다 생년이 늦은 기존 배우들의 성별 분포를 조회한다.
5) 남녀 수가 더 많은 쪽의 성별을 선택하고, 동일할 경우 난수를 이용해 성별을 결정한다.
