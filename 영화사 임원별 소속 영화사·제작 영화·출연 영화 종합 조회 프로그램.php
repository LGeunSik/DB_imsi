...
/*
[PHP 프로그램 제목]
영화사 임원별 소속 영화사·제작 영화·출연 영화 종합 조회 프로그램

[코드 동작 흐름 설명]
1) movieexec 테이블에서 모든 임원 정보를 이름 순으로 조회한다.
2) 각 임원(certno)에 대해 사장으로 재직 중인 영화사를 studio 테이블에서 조회한다.
3) 해당 임원이 제작한 영화 목록을 movie 테이블에서 연도 순으로 조회한다.
4) 해당 임원이 배우로 출연한 영화 목록을 starsin 테이블에서 조회한다.
5) 영화사, 제작 영화, 출연 영화 목록의 개수를 비교하여 최대 행 수를 계산한다.
6) rowspan을 활용해 임원 기본 정보는 한 번만 출력하고, 관련 목록은 행 단위로 정렬 출력한다.
7) 모든 임원에 대해 위 과정을 반복하여 하나의 HTML 테이블로 결과를 출력한다.
*/
